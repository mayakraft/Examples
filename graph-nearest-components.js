...
svg.size(1, 1).padding(0.03).strokeWidth(0.015);
// draw the graph
const drawing = svg.graph(graph);
// style
drawing.edges.mountain.stroke("#158");
drawing.edges.valley.stroke("#e53");
// drawing.faces.fill("none").stroke("none");
...
svg.onMove = (e) => {
  // reset all styles
  drawing.vertices.childNodes.forEach(vertex => vertex.fill("none").stroke("none"));
  drawing.faces.childNodes.forEach(face => face.fill("none"));
  edgeLayer.removeChildren();
  // get the indices of the nearest components
  const vertex = ear.graph.nearest_vertex(graph, e.position);
  const edge = ear.graph.nearest_edge(graph, e.position);
  const face = ear.graph.nearest_face(graph, e.position);
  if (vertex !== undefined) {
    drawing.vertices.childNodes[vertex].stroke("#158").fill("#fb4");
  }
  if (edge !== undefined) {
  ...
  if (face !== undefined) {
    drawing.faces.childNodes[face].fill("#fb4");
  }
};
