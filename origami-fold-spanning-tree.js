...
const draw = (graph, face) => {
  ...
  const drawing = foldedLayer.origami(ear.graph(graph).flatFolded(face), false)
    .translate(1.2, 0);
  const selectedFace = drawing.faces.children[face].fill("#e53").opacity(1);
   selectedFace.remove();
  drawing.faces.appendChild(selectedFace);
};
...
const load = (graph) => {
  ...
  const faded = bottomLayer.origami(crane)
    .opacity(0.1)
    .translate(1.2,0);
  faded.edges.mountain.stroke("black");
  faded.edges.valley.stroke("black");

  draw(crane, start_face);
};
