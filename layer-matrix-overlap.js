...
const draw_matrix = (matrix) => {
  ...
  const colorMap = { "false": "white", "true": "#fb4" };
...
const load = (FOLD) => {
  ...
  const facePolys = foldedFaces
    .map(face => ear.svg.polygon(face)
      .fill("#fb4")
      .appendTo(facesLayer));
